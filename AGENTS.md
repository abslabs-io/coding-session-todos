...
- `package.json` `contributes` — View container `claudeTodos`, view `claudeTodos.list`, `claudeTodos.refresh` command, and the `viewsWelcome` gated on the `claudeTodos.state` context key.
...
- **Multi-session, not single-session.** `findActiveSessions` returns every transcript with mtime within `ACTIVE_WINDOW_MS` (30 min). The view shows all of them, sorted by mtime descending. The workspace-cwd session (if present) drives the view title and badge; the others are still rendered.
...
- **Two-timer refresh.** `fs.watch` per session feeds `scheduleRefresh` with a `REFRESH_THROTTLE_MS` (5s) coalesce. Separately, while any session is `active`, `syncStateTickers` schedules a one-shot reload `STATE_TICK_MS` (6s) out so the spinner can flip to check/warning even when nothing else writes to the transcript.
