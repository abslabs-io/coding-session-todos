...
- **Status-bar session selection.** The widget reflects the session sitting exactly at the workspace folder, else the most recently active session running _inside_ it (`isWithinWorkspace` — workspace folder or a descendant cwd; uses `path.relative` so siblings like `app2` don't match `app`). Context % shows even when a session has no todos (`Claude · NN%`). The item is always visible: with no in-workspace session it shows just `$(checklist)`, and the tooltip always enumerates _every_ active session across projects, in-workspace or not.
